...
def make_board(width=18, height=10):
    tile = 66
...
class BoardState(ComputerAI):
    ...
    def shift_map_pane(self):
        ...
        (xmin, xmax, ymin, ymax) = self.map_panes[self.pane] # xmin xmax ymin ymax
        (x,y) = self.active_spot
        margin = 2
        # left/right moves only
        if y < (ymax - margin) and y > (ymin + margin):
            if x > (xmax - margin) and self.pane == 'nw':
                self.pane = 'ne1'
            elif x < (xmin + margin) and self.pane == 'ne1':
                self.pane = 'nw'
            elif x > (xmax - margin) and self.pane == 'ne1':
                self.pane = 'ne2'
            elif x < (xmin + margin) and self.pane == 'ne2':
                self.pane = 'ne1'
            elif x > (xmax - margin) and self.pane == 'ne2':
                self.pane = 'ne3'
            elif x < (xmin + margin) and self.pane == 'ne3':
                self.pane = 'ne2'

            elif x > (xmax - margin) and self.pane == 'sw':
                self.pane = 'se1'
            elif x < (xmin + margin) and self.pane == 'se1':
                self.pane = 'sw'
            elif x > (xmax - margin) and self.pane == 'se1':
                self.pane = 'se2'
            elif x < (xmin + margin) and self.pane == 'se2':
                self.pane = 'se1'
            elif x > (xmax - margin) and self.pane == 'se2':
                self.pane = 'se3'
            elif x < (xmin + margin) and self.pane == 'se3':
                self.pane = 'se2'
        ...
        if y < (ymin + margin):
            if self.pane == 'sw':
                ...
                self.pane = 'ne3'
        # BUGGY in CORNERS -- does not combine. does not scale to arbitrary map sizes.
        (xmin, xmax, ymin, ymax) = self.map_panes[self.pane] # xmin xmax ymin ymax
