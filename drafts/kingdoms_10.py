...
[x] 10. capture army works - moves to self.dead_armies
[x] 10. enemies still not moving towards army well yet.
...
- detect 2 or 3 hexes away consistently (for ranged)
- add ranged attack
- add naval

- allow view scrolling (enter a sub-loop where keys respond but locks out all other moves until ESC)
- if enemy path crosses water, enemy doesnt store up movement points to cross
...
def game_loop():    
    ...
    while True:
        ...
        if bs.active_spot == (0,0): # this hack deals with first move bug
            ...
            bs.next_army()
            bs.menu_display(["Begin battle", "press any key"], "Yellow", wait=0.5)
            bs.shift_map_pane()
        for event in pygame.event.get():
            keys = pygame.key.get_pressed()
            if event.type == pygame.QUIT:
