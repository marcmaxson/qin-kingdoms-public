...
def create_armies():
    ...
    army_spots = [(1,1)] #, (4,4), (6,6)] # starting spots
    enemy_spots = [] #[(8,3)] #, (6,4)]
...
class BoardState(ComputerAI):
    ...
    def __init__(self, armies=None, **kwargs):
        ...
        self.view_port = (3,3) # 7 x 7 hex view
        self.view_margin = 1
        self.view_x_min = 0
        self.view_x_max = 2
        self.view_y_min = 0
    ...
    def camera_pos_to_map_pos0(self, camera_x, camera_y):
        # instead of using draw() on MaxHex pieces, this shifts and blits them within the viewport
        map_x = camera_x + self.view_x_min
        map_y = camera_y + self.view_y_min
        return (map_x, map_y)
        
    def draw_map0(self):
        for piece in self.on_screen():
            map_x, map_y = self.camera_pos_to_map_pos0(piece[0], piece[1])
            surf.blit(self.board[piece].image, (self.edge + (map_x*self.tile), self.offset(map_x) + self.edge + (map_y*self.tile)))
        pygame.display.flip()
    ...
    def attack(self, enemy):
        ...
        self.active_army.draw()
        enemy.draw()
    def center_view_on(self, map_spot):
        ...
        (x,y) = map_spot
        current_view_x_center = self.view_x_max - self.view_margin #)/2 +1
        current_view_y_center = self.view_y_max - self.view_margin #)/2 +1
        ...
        print('new shift', self.view_x_min, self.view_y_min)
    ...
    def draw_tile(self, camera_x, camera_y):
        this_tile = self.get_tile(map_x, map_y)
        pass

    def camera_pos_to_map_pos(self, camera_x, camera_y):
        # instead of using draw() on MaxHex pieces, this shifts and blits them within the viewport
        map_x = camera_x + self.view_x_min
        map_y = camera_y + self.view_y_min
        return (map_x, map_y)
    ...
    def draw_map(self):
        camera_width = self.view_port[0]
        camera_height = self.view_port[1]
        for camera_x in range(0, camera_width):
            for camera_y in range(0, camera_height):
                map_x, map_y = self.camera_pos_to_map_pos(camera_x, camera_y)
                self.draw_tile(camera_x, camera_y, get_tile(map_x, map_y))        
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and sum(keys) > 0:                
                ...
                if successful:
                    bs.center_view_on(bs.active_spot)
                    bs.draw_map0()
                    #cursor = ActiveCursor(*bs.active_spot)
                    [a.draw() for a in bs.armies]                
