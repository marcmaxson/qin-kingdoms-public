...
class BoardState(ComputerAI):
    ...
    def draw_map(self):
        ...
        [a.draw(self.view_x_min, self.view_y_min) for a in self.armies]
        pygame.display.flip()
...
def game_loop():
    ...
    while True:
        ...
        if bs.active_army.owner != 0:
            ...
            # TODO: scroll screen to enemy
            # ADD AI MOVE FUNC HERE
            bs.shift_map_pane()
            bs.AI_move()
            bs.next_army()
            cursor.update(bs.active_spot, bs.view_x_min, bs.view_y_min)
            # TODO: if army not on board, find board army IS on and shift there 
            bs.shift_map_pane()
