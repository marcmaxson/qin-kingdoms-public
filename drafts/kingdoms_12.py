...
- TODO: actually test all_castles_filled, all invaders dead
- add fire
- add ballistas (3 hexes away)
- fix AI: blocked by other army (recalc and go around)
- view: mouse over army should show stats for that army
- display possible move keys below army
- add barricade (pikemen bonus)
...
class BoardState(ComputerAI, SideMenu):
    ...
    def move_army(self, keys):
        ...
        Moves:
            [x] attack, [x] mobilize, [x] view [x] range_attack
            [x] add attack for ranged units automatically expands possible squares with shaded overlay
        - add all other possible move commands: flee, barricade, fire        
        ...
        if self.has_army((x,y)) and (x != self.active_spot[0] or y != self.active_spot[1]):
            ...
            if this_army.owner != 0:
                ...
                self.menu_display([f"{self.active_army.gen.name} attacking",
                                   ...
                                   f"{self.active_army.attack} vs {this_army.defense}",
                                   f"{self.active_army.gen.war} vs {this_army.gen.war}",
    ...
    def next_army(self):
        ...
        if self.a > (len(self.armies)-1):
            self.turn += 1
            self.menu_display(f'Next turn: {self.turn}', ephemeral=True, replace=True, wait=0.5)
            self.a = 0            
            # check if battle time is up:
            if self.turn > 21:
                self.banner([f'Battle is over',"21 days are up"])
                self.clock.tick(0.33)
                pygame.quit()
                sys.exit()
        ...
        if player_remaining is False:
            ...
            pygame.quit()
            sys.exit()
        # all castles occupied by invader
        if self.all_castles_occupied():
            self.banner(['Battle is over:',"Invading army controls", "all the castles"])
            self.clock.tick(0.33)
            pygame.quit()
            sys.exit()            
    ...
    def attack(self, enemy):
        ...
        # CAPTURED?
        if enemy.size == 0:
            self.menu_display([f"{enemy.gen.name}","has been captured!"], "Red", wait=0.5)
        ...
        if self.active_army.size == 0:            
            self.menu_display([f"{self.active_army.gen.name}","has been captured!"], "Red", wait=0.5)
    ...
    def view_mode_movement(self, keys):
        ...
        if keys[pygame.K_7]:
            ...
            nxt = True
        elif keys[pygame.K_SPACE] or keys[pygame.K_5] or keys[pygame.K_ESCAPE]:
            return False
        if not self.on_board((x,y)):
            return
        self.active_spot = (x,y)
    ...
    def range_attack(self):
        ...
        # TODO add enemy barricade defense 
        def_bonus = self.terrain_defense[self.terrain_spot(enemy.spot())]
        # TODO add FOREST RANGE DEFENSE BONUS
        army_ratio = self.active_army.range_attack / enemy.defense
        war_ratio = (self.active_army.gen.war/enemy.gen.war)
        if war_ratio > 1:
            war_ratio = war_ratio/(math.sqrt(war_ratio)) # approx divides by max 3
        elif war_ratio <= 1:
        ...
        player_attack = war_ratio * army_ratio
        enemy_defense = 1/player_attack * def_bonus
        total_loss = 500 if self.active_army.gen.war > 50 else (300 + (self.active_army.gen.war/100)*200)
...
def game_loop():    
    ...
    while True:
        ...
        if bs.active_spot == (0,0): # this hack deals with first move bug
            ...
            bs.next_army()
            bs.menu_display(["Begin battle", "press any key"], "Yellow", wait=2)
            bs.shift_map_pane()
        for event in pygame.event.get():
            keys = pygame.key.get_pressed()
            if event.type == pygame.QUIT:
            ...
            if event.type == pygame.KEYDOWN:
                if bs.active_army.owner == 0:
                    ...
                    bs.cursor.update(bs.active_spot, bs.view_x_min, bs.view_y_min)
        if bs.active_army.owner != 0:
            print(f"DEBUG {bs.active_army.gen.name}'s turn")
            bs.shift_map_pane()
            bs.clock.tick(2)
            bs.AI_choose_action()
            bs.AI_move()
